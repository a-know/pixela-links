...
23:59 付近に sync が発火し、その直後から日付が変わるまでの間に増加したデータ（例：歩数）は Pixela に送信されない可能性がある。これは以下の理由による。

- 日付変更後の `fetchTodayTotal()` は新しい日付の累計のみを返す
- `requiresReset` が `true` になるため前日の `lastSentValue` との差分ではなく当日累計をそのまま送信する
- 結果として、日付変更直前の未送信分は前日・当日どちらのグラフにも反映されない

**定時起動（例：毎日 23:59）による回避策の検討結果：**
iOS の `BGAppRefreshTask` は `earliestBeginDate` で最短起動時刻を指定できるが、実際の起動タイミングは OS の裁量（バッテリー・使用パターン等）であり、特定時刻の起動は保証されない。サーバーからのサイレントプッシュ（APNs）を使えば精度は上がるがバックエンド構築が必要になる。

**方針：初期バージョンでは仕様として許容する。**
HealthKit バックグラウンド配信（`frequency: .immediate`）は通常数分間隔で発火するため、日付変更直前の未送信ウィンドウは非常に小さく、実用上の影響は軽微と判断した。
